 ...
 */
export function UTF8ArrayToString(heapOrArray, idx = 0, maxBytesToRead = Number.NaN) {
    // 1. Input handling
    ...
    while (readAt(heapOrArray, endPtr) && !(endPtr >= endIdx))
        ++endPtr;
    // 2. Core processing - use TextDecoder for longer strings if available
    if (endPtr - idx > TEXT_DECODER_THRESHOLD &&
    ...
    while (idx < endPtr) {
        ...
        if ((byte0 & UTF8_BYTE_MASKS.TWO_BYTE_MASK) ===
            UTF8_BYTE_MASKS.TWO_BYTE_PREFIX) {
            // Two-byte character
            decodedString += String.fromCharCode(((byte0 & UTF8_BYTE_MASKS.TWO_BYTE_DATA_MASK) << 6) | byte1);
            continue;
        }
        const byte2 = readAt(heapOrArray, idx++) & UTF8_BYTE_MASKS.CONTINUATION_MASK;
        if ((byte0 & UTF8_BYTE_MASKS.THREE_BYTE_MASK) ===
        ...
        if (byte0 < UTF8_CONSTANTS.MAX_BMP) {
            ...
            decodedString += String.fromCharCode(UTF8_CONSTANTS.HIGH_SURROGATE_START | (codePoint >> 10), UTF8_CONSTANTS.LOW_SURROGATE_START |
...
export const lengthBytesUTF8 = (str) => {
    ...
    for (let i = 0; i < str.length; ++i) {
        ...
        else if (charCode >= UTF8_CONSTANTS.HIGH_SURROGATE_START &&
...
export const stringToUTF8Array = (str, heap, outIdx, maxBytesToWrite) => {
    ...
    for (let i = 0; i < str.length; ++i) {
        ...
        if (codePoint >= UTF8_CONSTANTS.HIGH_SURROGATE_START &&
        ...
        if (codePoint <= UTF8_CONSTANTS.MAX_SINGLE_BYTE) {
            ...
            writeAt(heap, outIdx++, UTF8_BYTE_MASKS.CONTINUATION_PREFIX |
...
export function intArrayFromString(stringy, dontAddNull, length) {
    // 1. Input handling
    const byteLength = length && length > 0 ? length : lengthBytesUTF8(stringy) + 1;
    const byteArray = new Array(byteLength);
    // 2. Core processing
    const numBytesWritten = stringToUTF8Array(stringy, byteArray, 0, byteArray.length);
    // 3. Output handling
    if (dontAddNull)
        byteArray.length = numBytesWritten;
    return byteArray;
}
