...
import { toss } from "./environment.mjs";

/**
 * SharedArrayBuffer backed serializer used to exchange parameters and
 * exceptions with the controller thread.
 *
 * @module async-proxy/serialization-buffer
 */

/**
 * @typedef {import('./serialization-buffer.d.ts').SerializableValue} SerializableValue
 */

export class SerializationBuffer {
    /**
     * @param {import('./serialization-buffer.d.ts').SerializationBufferInit} options -
    ...
    constructor({ sharedBuffer, offset, size, littleEndian, exceptionVerbosity }) {
         ...
         * @type {Record<string, import('./serialization-buffer.d.ts').TypeDescriptor>}
         */
        this.typeInfo = {
     ...
     *
     * @param {...SerializableValue} values - Values stored for the consumer.
     */
    serialize(...values) {
        if (!values.length) {
            this.bytes[0] = 0;
     ...
     * @returns {SerializableValue[]} Payload values.
     */
    ...
    /**
     * Conditionally serializes an exception string based on the configured
     * threshold.
     *
     * @param {number} priority - Smaller numbers represent higher priority.
     * @param {import('./serialization-buffer.d.ts').SerializableError} error -
     *   Error object to stringify.
     */
