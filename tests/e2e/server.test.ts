...
beforeAll(async () => {
    ...
    server = spawn('npx', ['tsx', 'scripts/http-service.ts', './', '--port', String(PORT)], {
        stdio: 'pipe',
        shell: true, 
    });
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        // Timeout
        setTimeout(() => reject(new Error(`Server start timeout. Output so far: ${output}`)), 10000);
    });
...
test('should load samples/index.html and initialize sqlite3 without "Invalid URL" error', async () => {
    ...
    try {
        await expect.poll(() => consoleLogs, { timeout: 10000, interval: 500 }).toContain('init sqlite3');
    } catch (e) {
    ...
    const invalidUrlErrors = consoleErrors.filter(e => e.includes("Invalid URL"));
